    ...
    if ((HAL_GetTick() - adctmr) >= ADC_READOUT_PERIOD){
        ...
        if      (adc_cnt == 0U){
            ...
            if((tmp < 100) || (tmp > 4000)){
                ...
                if((thst.mv_temp / 10) != (ntc_temp / 10))
                {
                    if ( (HAL_GetTick() - temp_compare_lastTick) <= TEMP_COMPARE_TIME)
                    {
                        if( (HAL_GetTick() - temp_compare_betweenTick) >= ( (TEMP_COMPARE_TIME - 500) / 10) )
                        {
                            temp_compare_betweenTick = HAL_GetTick();
                            
                            if( (ntc_temp_old / 10) != (ntc_temp / 10) )
                            {
                                ntc_temp_old = ntc_temp;
                                
                                temp_difference_counter = 0;
                            }
                            else
                            {
                                ++temp_difference_counter;
                                
                                if(temp_difference_counter == TEMP_DIFF_COUNT_MAX)
                                {
                                    temp_difference_counter = 0;
                    ...
                    {
                        temp_compare_lastTick = HAL_GetTick();
                        temp_compare_betweenTick = HAL_GetTick();
