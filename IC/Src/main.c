...
#define LSE_TIMEOUT 1000                    // Timeout za proveru stanja oscilatora (u milisekundama)
/* Private Variable ----------------------------------------------------------*/
...
#define TEMP_COMPARE_TIME 3000
#define TEMP_DIFF_COUNT_MAX 10
uint32_t temp_compare_lastTick = 0, temp_compare_betweenTick = 0;
int16_t ntc_temp_old = 0;
uint8_t temp_difference_counter = 0;
...
void CheckRTC_Clock(void) {
    ...
    // Proveri da li je prošlo dovoljno vremena za novu proveru
    if ((HAL_GetTick() - lastCheckTime) >= 1100) {  // 1 sekunda interval
        lastCheckTime = HAL_GetTick();  // Ažuriraj vreme poslednje provere
        // Proveri trenutni broj sekundi RTC-a
        ...
        if (!LSE_Failed) {
            ...
            // Ako se broj sekundi nije promenio od poslednje provere, znaci da LSE nije funkcionalan
            ...
            // Ažuriraj poslednje sekundno ocitavanje
