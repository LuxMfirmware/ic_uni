...
TF_Result JALOUSIE_SET_Listener(TinyFrame *tf, TF_Msg *msg)
{
    ...
    {
        // provjeri prvo jel naštimana roletna u meniju
        bool isZero = (curtains[i].relayUp == 0 || curtains[i].relayDown == 0);
        // uradi sve provjere na nule, adrese i smjer
        if(adr && !isZero && (adr == curtains[i].relayUp) && (dir == 1))
        {
            // promjeni flag žaluzine na gore curtains[i].extUp
        }
        else if(adr && !isZero && (adr == curtains[i].relayDown) && (dir == 2))
        {
            // promjeni flag žaluzine na dole curtains[i].extDown
        }
    }
    return TF_STAY;
}
...
TF_Result THERMOSTAT_GET_Listener(TinyFrame *tf, TF_Msg *msg)
{
    ...
    if(thst.master && thst.group == msg->data[0])
    {
        ...
        resp[7] = thst.sp_max;
        resp[8] = thst.sp_diff;
        resp[9] = thst.fan_speed;
        resp[10] = thst.fan_loband;
        resp[11] = thst.fan_hiband;
        resp[12] = thst.fan_diff;
        resp[13] = thst.fan_ctrl;
        resp[14] = thst.fan_quiet_start;
        resp[15] = thst.fan_quiet_end;
        resp[16] = thst.fan_quiet_speed;
        msg->len = 17;
        msg->data = resp;
        TF_Respond(tf, msg);
    }
...
TF_Result THERMOSTAT_INFO_Listener(TinyFrame *tf, TF_Msg *msg)
{
    ...
    if(thst.group == msg->data[0])
    {
        ...
        if(thst.master) {
            ...
            // ako je slave imao zahtjev za sinhronizaciju onda je red da se ogovori
//            if(all_zero){
...
/**
* @brief :  ovo je ID listener registrovan za sve upite, takav nacin mogucava više
...
/**
* @brief :  Ubaci sledecu komandu u red komandi na cekanju
* @param :
...
void RS485_Service(void)
{
    ...
    // šalji komande na redu ack_flag je ujedno i kontrola
    // slanja dok prethodni nije potvrdan ne ide sledeci tako
    // da ako neko zabrlja a hoce treba odlucit šta uradit ili slat
    // sledeceg i neuspjelog preskocit ili ponovo stavit u red
